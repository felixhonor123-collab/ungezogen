...
export class QrScannerComponent implements AfterViewInit, OnDestroy {
  ...
  startScanner() {
    if (this.useFileInput) {
      setTimeout(() => this.fileInputRef?.nativeElement.click(), 50);
      return;
    }
    this.error.set(null);
    ...
    navigator.mediaDevices
      ...
      .then((stream) => {
        ...
        video.onloadedmetadata = () => {
          video.play().then(() => this.scheduleDetect()).catch(() => {
            this.scanning.set(false);
            this.error.set('Kamera konnte nicht gestartet werden.');
          });
        };
      })
      .catch(() => {
        this.scanning.set(false);
        this.error.set('Kamera konnte nicht geöffnet werden. Bitte Kamerazugriff erlauben.');
      });
  }
  ...
  onFileSelected(event: Event) {
    ...
    if (!file) {
      if (this.useFileInput) this.closed.emit();
      return;
    }
