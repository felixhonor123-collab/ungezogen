...
export class Home implements OnInit {
  ...
  readonly items: Item[] = [
    { label: 'Pizza Margherita', price: 9.5 },
    { label: 'Kino-Ticket', price: 13.0 },
    { label: 'Mittagessen', price: 12.8 },
    { label: 'Tankfüllung', price: 78.4 },
    { label: 'Konzertkarte', price: 55.0 },
    { label: 'Supermarkt-Einkauf', price: 34.2 },
    { label: 'Uber-Fahrt', price: 18.6 },
    { label: 'Restaurant-Rechnung', price: 47.9 },
  ];
  ...
  ngOnInit() {
    this.schuldService.getBenutzer().subscribe((liste) => {
      this.benutzerListe.set(liste);
      this.laden.set(false);
      const benutzerId = this.route.snapshot.queryParamMap.get('benutzerId');
      if (benutzerId) {
        const gefunden = liste.find((b) => b.id === benutzerId);
        if (gefunden) {
